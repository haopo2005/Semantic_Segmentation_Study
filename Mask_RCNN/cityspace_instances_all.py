...
if __name__ == '__main__':
    ...
    for imgpath in imglists:
        ...
        result = '/home/jst/share/instances/'+path[len(path)-1]
        [h,w,c] = np.shape(masks)
        ff = open(result+'.txt','w+')
        for k in range(c):
            loc = np.where(masks[:,:,k]==1)
            final_mask = np.zeros([h,w], dtype=np.uint8)
            y = loc[0]
            x = loc[1]
            final_mask[y,x] = 255 #only for test,non-zeros is required
            cv2.imwrite(result + str(png_count) + '.png', final_mask)
            id = name2label[class_names[class_ids[k]]].id 
            png_count = png_count + 1
            ff.write(result + str(png_count) + '.png'+' '+str(id)+' '+ str(scores[k])+'\n')
        ff.close()
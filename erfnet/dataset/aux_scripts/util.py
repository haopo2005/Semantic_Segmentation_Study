...
def prediction_to_grey(predicted_mask, label_remap, label_idmap):
  # get prediction and make it color
  # map to color
  grey_mask = np.zeros([predicted_mask.shape[0], predicted_mask.shape[1], 1])
  for key in label_remap:
    grey_mask[np.where((predicted_mask == label_remap[key]))] = label_idmap[key]
  return grey_mask

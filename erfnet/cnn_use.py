...
def predict_mask(img, net, FLAGS, DATA):
  ...
  # change to color
  color_mask = util.prediction_to_grey(
      mask, DATA["label_remap"], DATA["label_idmap"])
  
  res = cv2.resize(color_mask,(2048,1024),interpolation = cv2.INTER_NEAREST)
  ...
  cv2.imwrite(FLAGS.log + "/" + os.path.basename(img), res)
